...
export default function NavBar({
    ...
    const linkStyle = NavBarVariants[variant].linkText;
    const buttonStyle = NavBarVariants[variant].button;

    return (
        <nav aria-label="navigation bar" className={`${baseNavContainerStyle} ${navContainerStyle}`}>
            <div className={"flex items-center mr-auto"}>
                ...
                {isDefault &&
                    ...
                    {links.length !== 0 &&
                        links.map((link) => {
                            return (
                                <Link key={link.label} href={link.href} className={`${linkStyle}`}>{link.label}</Link>
                            )
                        })
                    }
                    </div>
            ...
            <div className={"ml-auto"}>
                <Button className={`${buttonStyle}`} variant={"outline"}>
                    {isDefault ?
                        "Download"
                        : <svg aria-label="open menu icon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
